...
public class SpawnZone_20_9 : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider other)
    {
        if (other.gameObject.name.Substring(0, 4) == "Ball")
        {
            ...
            if (!Input.GetMouseButton(0))
            {
                ...
                if (gameObject.name == "SpawnLeft" && other.gameObject.transform.position.z != 7.91f)
                {
                    BallPos = other.gameObject.transform.position;
                    BallPos.z = 7.91f;
                    other.gameObject.transform.position = BallPos;
                }
                if (gameObject.name == "SpawnRight" && other.gameObject.transform.position.z != 11.69f)
                {
                    BallPos = other.gameObject.transform.position;
                    BallPos.z = 11.69f;
                    other.gameObject.transform.position = BallPos;
